...
onmessage=function(e){
    ...
    try{
        ...
        if(!img || img.width===0 || img.height===0){ postMessage("RECOVER"); return; }
        ...
        if(img.width!==currentW || img.height!==currentH){
            ...
            mask=new cv.Mat(img.height,img.width,cv.CV_8UC4,new Uint8ClampedArray(img.height*img.width*4).fill(0));
        ...
        let bVal=blur||3; if(bVal%2===0) bVal+=1;
        ...
        if(panel==='A'){
            ...
            let otsuVal=cv.threshold(edges,tempMat,0,255,cv.THRESH_BINARY+cv.THRESH_OTSU)[0];
            tempMat.delete();
            ...
            if(oldCode){
                ...
                let tempMat=new cv.Mat();
                cv.threshold(edges,tempMat,0,255,cv.THRESH_BINARY+cv.THRESH_OTSU);
                tempMat.delete();
            } else {
