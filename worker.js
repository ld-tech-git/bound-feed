...
onmessage = function(e){
    ...
    try{
        ...
        if(img.width!==currentW || img.height!==currentH){
            if(src) src.delete(); if(gray) gray.delete(); if(blurred) blurred.delete(); if(edges) edges.delete(); if(mask) mask.delete();
            src = new cv.Mat(img.height, img.width, cv.CV_8UC4);
            gray = new cv.Mat();
            blurred = new cv.Mat();
            edges = new cv.Mat();
            mask = new cv.Mat(img.height,img.width,cv.CV_8UC4,new Uint8ClampedArray(img.data.length));
